...
__doc__      = 'Plugin voor QGis om metadata aan te maken volgens het Nederlandse ISO profiel 19115 2.0.0'
...
from .ME_19115_200_ui import Ui_ME_19115_200
...
class ME_19115_200(QDialog):
  def __init__(self, parent):
    ...
    csv_bestand = self.eigenlijsten["dirs"]["csv_dir"].replace('%USERNAME%', os.getenv('USERNAME'))+os.sep+"Metadata_Master.csv" 
    ...
    elif self.iface.activeLayer().type() == self.laagType[0]:
      self.ui.cbx_Ruimtelijke_schema.setCurrentIndex(addItemLijst.index("vector"))
    elif self.iface.activeLayer().type() == self.laagType[1]:
    ...
    elif self.iface.activeLayer().type() != self.laagType[0] and self.eigenlijsten["Voorkeuren"]["cbx_Ruimtelijke_schema"] in addItemLijst:
    ...
    elif self.iface.activeLayer().type() == self.laagType[0] and self.iface.activeLayer().dataProvider().encoding().lower().replace("-", "") in addItemLijst:
    ...
    # lees de A18 en A19 codelijsten uit
    self.A18_codelijst = [self.codelijsten['A.18 Codelijst Protocol'][index][1] for index in range(len(self.codelijsten['A.18 Codelijst Protocol']))]
    self.A19_codelijst = [self.codelijsten['A.19 Codelijst media types'][index][1] for index in range(len(self.codelijsten['A.19 Codelijst media types']))]
    ...
    if hasattr(self, 'onlineOmschrijving_1') and self.onlineOmschrijving_1 in addItemLijst_omschrijving: 
    ...
    # als onlineOmschrijving = endPoint -> A.19 Codelijst media types en de voorkeur = cbx_protocol_A19
    if self.ui.cbx_omschrijving_1.currentText() == 'endPoint': 
      addItemLijst = self.A19_codelijst
      cbx_protocol_1 = "cbx_protocol_A19"
    # als onlineOmschrijving = accessPoint -> A.18 Codelijst Protocol en de voorkeur = cbx_protocol_A18
    elif self.ui.cbx_omschrijving_1.currentText() == 'accessPoint': 
      addItemLijst = self.A18_codelijst
      cbx_protocol_1 = "cbx_protocol_A18"
    ...
    if hasattr(self, 'onlineProtocol_1') and self.onlineProtocol_1 in addItemLijst: 
      self.ui.cbx_protocol_1.setCurrentIndex(addItemLijst.index(self.onlineProtocol_1))
    # neem anders de voorkeur
    elif self.eigenlijsten["Voorkeuren"][cbx_protocol_1] in addItemLijst:
      self.ui.cbx_protocol_1.setCurrentIndex(addItemLijst.index(self.eigenlijsten['Voorkeuren'][cbx_protocol_1]))
    # als in de URL op enter wordt gedrukt start dan de functie vulaan_URL_gegevens
    ...
    for num in range(2, 6):
      ...
      exec(exec_tekst)
      exec_tekst = """if self.ui.cbx_omschrijving_%s.currentText() == 'endPoint': \
      self.ui.cbx_protocol_%s.addItems(self.A19_codelijst)""" %(num, num)
      exec(exec_tekst)
      exec_tekst = """if self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint': \
      self.ui.cbx_protocol_%s.addItems(self.A18_codelijst)""" %(num, num)
      exec(exec_tekst)
      ...
      exec_tekst = """if hasattr(self, 'onlineProtocol_%s') and self.ui.cbx_omschrijving_%s.currentText() == 'endPoint': \
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A19_codelijst.index(self.onlineProtocol_%s))""" %(num, num, num, num)
      exec(exec_tekst)
      exec_tekst = """if hasattr(self, 'onlineProtocol_%s') and self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint': \
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A18_codelijst.index(self.onlineProtocol_%s))""" %(num, num, num, num)
      exec(exec_tekst)
      exec_tekst = """if not hasattr(self, 'onlineProtocol_%s') and self.ui.cbx_omschrijving_%s.currentText() == 'endPoint' and self.eigenlijsten["Voorkeuren"]["cbx_protocol_A19"] in self.A19_codelijst: \
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A19_codelijst.index(self.eigenlijsten['Voorkeuren']['cbx_protocol_A19']))""" %(num, num, num)
      exec(exec_tekst)
      exec_tekst = """if not hasattr(self, 'onlineProtocol_%s') and self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint' and self.eigenlijsten["Voorkeuren"]["cbx_protocol_A18"] in self.A18_codelijst: \
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A18_codelijst.index(self.eigenlijsten['Voorkeuren']['cbx_protocol_A18']))""" %(num, num, num)
      exec(exec_tekst)
    ...
    # als er iets veranderd in cbx_omschrijving verander dan cbx_protocol
    # zie: https://stackoverflow.com/questions/23116763/pyqt-how-to-connect-qcombobox-to-function-with-arguments
    slotLambda = lambda: self.OmschrijvingChanged(1)
    self.ui.cbx_omschrijving_1.currentIndexChanged.connect(slotLambda) 
    slotLambda = lambda: self.OmschrijvingChanged(2)
    self.ui.cbx_omschrijving_2.currentIndexChanged.connect(slotLambda)
    slotLambda = lambda: self.OmschrijvingChanged(3)
    self.ui.cbx_omschrijving_3.currentIndexChanged.connect(slotLambda)
    slotLambda = lambda: self.OmschrijvingChanged(4)
    self.ui.cbx_omschrijving_4.currentIndexChanged.connect(slotLambda)
    slotLambda = lambda: self.OmschrijvingChanged(5)
    self.ui.cbx_omschrijving_5.currentIndexChanged.connect(slotLambda)    

    # Algemeen Attribuutgegevens
    # Verberg het tabblad als het geen vector bestand is
  ...
  def bepaal_xml_naam(self):
    ...
    if self.iface.activeLayer().type() == self.laagType[0]:
      ...
      # als het een database is
        ...
        self.xml_map = self.eigenlijsten['dirs']['xml_dir'].replace('%USERNAME%', os.getenv('USERNAME'))
        laag_geg = {item.split('=')[0]: item.split('=')[1].replace('"', '').replace("'", "") for item in bron_gegevens.split(' ') if '=' in item}
        self.xml_naam = laag_geg['table']
    ...
    # als het een raster laag is
      ...
      self.xml_map, self.xml_naam = os.path.split(bron_gegevens)
  ...
  def OmschrijvingChanged(self, num):
    """ 
    Als de combobox omschrijving in algemeen distributie wordt aangepast wijzig dan ook de combobox protocol
    """
    # maak de combobox leeg
    exec('self.ui.cbx_protocol_%s.clear()' %(num))
    # vul de combobox protocol_ als de cbx_omschrijving = accessPoint
    exec_tekst = """if self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint': \
      self.ui.cbx_protocol_%s.addItems(self.A18_codelijst)""" %(num, num)
    exec(exec_tekst)
    # vul de combobox protocol_ als de cbx_omschrijving = endPoint
    exec_tekst = """if self.ui.cbx_omschrijving_%s.currentText() == 'endPoint': \
      self.ui.cbx_protocol_%s.addItems(self.A19_codelijst)""" %(num, num)
    exec(exec_tekst)
    # lees de voorkeur uit van cbx_protocol_A18 en plaats deze als index als die voorkomt in self.ui.cbx_protocol_
    exec_tekst = """if self.eigenlijsten["Voorkeuren"]['cbx_protocol_A18'] in self.A18_codelijst and self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint':
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A18_codelijst.index(self.eigenlijsten['Voorkeuren']['cbx_protocol_A18']))""" %(num, num)
    exec(exec_tekst)
    # lees de voorkeur uit van cbx_protocol_A19 en plaats deze als index als die voorkomt in self.ui.cbx_protocol_
    exec_tekst = """if self.eigenlijsten["Voorkeuren"]['cbx_protocol_A19'] in self.A19_codelijst and self.ui.cbx_omschrijving_%s.currentText() == 'endPoint':
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A19_codelijst.index(self.eigenlijsten['Voorkeuren']['cbx_protocol_A19']))""" %(num, num)
    exec(exec_tekst)
    # overschrijf de index als het onlineProtocol uit de xml is uitgelezen
    exec_tekst = """if hasattr(self, 'onlineProtocol_%s') and self.onlineProtocol_%s in self.A18_codelijst and self.ui.cbx_omschrijving_%s.currentText() == 'accessPoint': 
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A18_codelijst.index(self.onlineProtocol_%s))""" %(num, num, num, num, num)
    exec(exec_tekst)
    # overschrijf de index als het onlineProtocol uit de xml is uitgelezen
    exec_tekst = """if hasattr(self, 'onlineProtocol_%s') and self.onlineProtocol_%s in self.A19_codelijst and self.ui.cbx_omschrijving_%s.currentText() == 'endPoint': 
      self.ui.cbx_protocol_%s.setCurrentIndex(self.A19_codelijst.index(self.onlineProtocol_%s))""" %(num, num, num, num, num)
    exec(exec_tekst)
  ...
  def opslag_contactgegevens(self):
    ...
    else: csv_regels[list(self.csv_dict.keys()).index(self.ui.cbx_beheer_email.currentText())+1] = regel+'\n'
    # schrijf het bestand weg
    with open(csv_bestand, 'w') as csvfile: 
      for csv_regel in csv_regels: csvfile.writelines(csv_regel)
    QtWidgets.QMessageBox.warning(None, None, "De contact gegevens zijn op geslagen in: \n%s" %(csv_bestand)) 
  ...
  def leesXML(self):
    ...
    if os.path.isfile(self.xml_map+os.sep+self.xml_naam):
      ...
      # toegangsrestricties (set geeft een unieke list terug)
      ...
      self.checkbox_dict['toegangsrestricties'] = list(set([toegangsrestrictie.get('codeListValue') for toegangsrestrictie in toegangsrestricties]))
      # gebruiksrestricties (set geeft een unieke list terug)
      ...
      self.checkbox_dict['gebruiksrestricties'] = list(set([gebruiksrestrictie.get('codeListValue') for gebruiksrestrictie in gebruiksrestricties]))
      # overige beperkingen 
  ...
  def schrijfXML(self):
    ...
    for sleutel in self.checkbox_dict.keys():
      if sleutel in ['toegangsrestricties', 'gebruiksrestricties', 'datalicenties', 'gebruikscondities']: dummy = True
    if dummy:
      tag36 = etree.SubElement(tag1, 'resourceConstraints')
    ...
    dummy = False
    for sleutel in self.checkbox_dict.keys():
      if sleutel in ['publieketoegang']: dummy = True
    if dummy:
      tag41_1 = etree.SubElement(tag1, 'resourceConstraints')
      tag41_2 = etree.SubElement(tag41_1, 'MD_LegalConstraints')
      # toegangsrestricties
      if 'toegangsrestricties' in self.checkbox_dict.keys() and self.checkbox_dict['toegangsrestricties']:
        toegangsrestrictie = self.checkbox_dict['toegangsrestricties'][0]
        tag41_3 = etree.SubElement(tag41_2, 'accessConstraints')
        toegangsrestricties =  etree.SubElement(tag41_3, 'MD_RestrictionCode', \
        codeList='./resources/codeList.xml#MD_RestrictionCode', codeListValue='%s' %(toegangsrestrictie))
      # publieketoegang
      if 'publieketoegang' in self.checkbox_dict.keys() and self.checkbox_dict['publieketoegang']:
        tag41_4 = etree.SubElement(tag41_2, 'otherConstraints')
        ...
        publieketoegang = etree.SubElement(tag41_4, '{%s}Anchor' %(namespaces['gmx']))
    ...
    if self.ui.le_Email_distributie_1.text(): 
    ...
    if self.ui.le_prijsinformatie.text() or self.ui.le_orderprocedure.text() or self.ui.le_doorlooptijd.text() \
      ...
      if not self.ui.le_Email_distributie_1.text(): 
        tag448 = etree.SubElement(tag92, 'distributor')
        tag449 = etree.SubElement(tag448, 'MD_Distributor')
